#
# Takes the base graph and filter based on tissue and cell-line data.
...
tissueFilter <- function(graph, t, minNTPM = 0, maxNTPM = 1000000) {
...
#
# nTPM = Normalised Transcripts per Million
typeFilter <- function(graph, t, minNTPM = 0, maxNTPM = 1000000) {
...
#
# nTPM = Normalised Transcripts per Million
lineFilter <- function(graph, cl, minNTPM = 0, maxNTPM = 1000000) {
...
# # Sanity Checks
# sg <- tissueFilter(g, "stomach", 0, 50)
# head(V(sg))
# 
# sg <- typeFilter(g, "Adipocytes", 10, 20)
# head(V(sg))
# 
# head(cellLine[cellLine$Cell.line == "143B", ], n = 30)
# sg <- lineFilter(g, "143B", 0, 50)
# head(V(sg))
